...
(function (Grapher) {
    ...
    var MotionChart = (function () {
		...
		//constructs function for retrieving data
		...
		//constructs function for specfying data for label
		...
		//constructs function for specfying data for x axis
		...
		//constructs function for specfying data for y axis
		...
		//constructs function for specfying data determining radius size of circles
		...
		//constructs function for specfying data determining color of cirlces
		...
		//constructs function for specfying data determining startime
		...
		//constructs function for specfying data determining endtime
		...
		//constructs function for specfying scale of x axis
		...
		//constructs function for specfying scale of y axis
		...
		//constructs function for specfying scale of the circle color
		...
		//Specifices the size and speed of the time animation
        MotionChart.prototype.startTransition = function () {
            ...
            var duration = ((endTime - currentTime) * 38500) / (endTime - startTime);
		...
		//master function integrating the sliders
        ...
        MotionChart.prototype.createScales = function () {
            ...
            var xScale = this._xScale.domain(xDomain).range([.75 * maxRadius, this._width - (.5 * maxRadius) - maxLabelWidth]);
		//Controls the limits of the y graph
		this._xScale = this._xScale.domain([.26, .475]).range([0, this._width]).clamp(true);
		...
		// Regulates the size of the tick size for time slider
		...
		//Regulates color slider marigns and tick size
        MotionChart.prototype.createColorAxis = function () {
            if (this._colorDomain) {
                var w = this._width * 0.35;
        ...
        MotionChart.prototype.createRadiusAxis = function () {
			...
			// The "-1000" added by jvk, removes the Radius slider
            var y = Number(this._element.attr("height")) - 10000;
		...
		//Creates the timeslider and integrates the tick size and speed of animation
        MotionChart.prototype.createTimeSlider = function () {
            var width = parseFloat(this._element.attr("width"));
            var w = this._width * (this._colorDomain ? 0.32 : 0.64)*1.67;
            var x = width - this._margin.left - w;
		...
		//Converts date into format which can be used in slider
		...
		//checks whether date is string or number and passes along acccordingly
		...
		//integrates date data from data source with the radisu,x, y and color data on chart
